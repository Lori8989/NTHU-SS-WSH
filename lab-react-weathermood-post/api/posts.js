...
function _createVote(id, mood) {
    console.log("id:",id,"mood",mood);
    const posts = _listPosts().map(p => {
        // TODO: Check p
        console.log("p:",p);
        if (p.id === id && p[mood.toLowerCase() + 'Votes'] == 1) {
            p[mood.toLowerCase() + 'Votes'] = 0;//cancel
            if(p.voted === mood) p.voted = "none";
        }
        else if (p.id === id) {
            p[mood.toLowerCase() + 'Votes'] =1;//add new one
            p[p.voted.toLowerCase() + 'Votes'] = 0;//clean old
            p.voted = mood;
        }
        else{
            p[p.voted.toLowerCase() + 'Votes'] = 0;//clean old
